...
let rec effect = function (* 副作用の有無 (caml2html: elim_effect) *)
  | Let((x, _), e1, e2) -> x.[0] = '$' || effect e1 || effect e2
  | IfEq(_, _, e1, e2) | IfLE(_, _, e1, e2) -> effect e1 || effect e2
  | LetRec(_, e) | LetTuple(_, _, e) -> effect e
  | ExtFunApp(f, _) when f = "create_array" || f = "create_float_array" || f = "cos" || f = "sin" || f = "atan" || f = "tan" -> false
...
let rec f = function (* 不要定義削除ルーチン本体 (caml2html: elim_f) *)
  ...
  | Let((x, t), e1, e2) -> (* letの場合 (caml2html: elim_let) *)
      let e1' = f e1 in
      let e2' = f e2 in
      if x.[0] = '$' || effect e1' || S.mem x (fv e2') then Let((x, t), e1', e2') else

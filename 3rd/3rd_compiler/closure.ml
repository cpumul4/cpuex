...
let rec g env known = function (* クロージャ変換ルーチン本体 (caml2html: closure_g) *)
  ...
  | KNormal.LetRec({ KNormal.name = (x, t); KNormal.args = yts; KNormal.body = e1 }, e2) -> (* 関数定義の場合 (caml2html: closure_letrec) *)
	 ...
	 と仮定し、knownに追加してe1をクロージャ変換してみる *)
      ...
      let zs = S.filter (fun x -> not (M.mem x !Global.genv || x.[0] = '$')) (S.diff (fv e1') (S.of_list (List.map fst yts))) in
      let known', e1' =
	...
	(Format.eprintf "free variable(s) %s found in function %s@." (Id.pp_list (S.elements zs)) x;
	 ...
	 let e1' = g (M.add_list yts env') known e1 in
	 known, e1') in
      let zs = List.filter (fun x -> not (M.mem x !Global.genv || x.[0] = '$')) (S.elements (S.diff (fv e1') (S.add x (S.of_list (List.map fst yts))))) in (* 自由変数のリスト *)
...
let f e =
  toplevel := [];
  let e' = g M.empty (S.of_list ["create_array"; "create_float_array"; "cos"; "sin"; "atan"; "tan"; "print_char"; "read_int"; "read_float"]) e in

...
let rec target' src (dest, t) = function
  | Mov(x) when x = src && is_reg dest && not (List.mem src allglobalregs || List.mem dest allglobalregs) ->
      assert (t <> Type.Unit);
      assert (t <> Type.Float);
      false, [dest]
  | FMov(x) when x = src && is_reg dest && not (List.mem src allglobalfregs || List.mem dest allglobalfregs) ->
      assert (t = Type.Float);
      false, [dest]
...
let rec alloc dest cont regenv x t =
  ...
  let all =
    match t with
    | Type.Unit -> [reg_0] (* dummy *)
    | Type.Float -> allfregs
    | _ -> allregs in
  if all = [reg_0] then Alloc(reg_0) else (* [XX] ad hoc optimization *)
...
and g' dest cont regenv = function (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
  | Nop | Set _ | SetF _ | SetL _ | In | InF | Comment _ | Restore _ as exp -> (Ans(exp), regenv)
